%%   DATE UPDATED: March 2nd, 2016
 
%%   CLEANED UP GLACIER MODEL FOR LAKE CREEK
%
%    1D FTCS STAGGERED GRID NUMERICAL MODEL: LAKE CREEK PALEOGLACIER
%
%    SI units
%
...
%
    clear global
%    close all  % if another program was previously running
...
%  set up distance array
    ...
    xmax  = 33880;     % make sure this matches your x-data !!!
       dx = xmax/step;
        x = dx/2:dx:xmax-(dx/2);
    xedge = 0:dx:xmax;
...
W0    = 1000*tributaries;
Wstar = 12000; % how quickly does the geometry shrink? SLOWER THAN NORMAL
...
Wedge = [Wedge(1) Wedge Wedge(end)];
    ...
    % zb = transpose(smooth(LC_new_profile(1:5:end))); 
...
%  meteorology and mass balance
    ...
    ELA0      = 3400;      % SET THE MIDDLE ELA
    
    
    sigma_ELA = 200;       % how much focing does the ELA feel?
    ...
    bcap = 1.2;            % m/yr, usually 1.25-2.00
...
if rem(t(i),tplot)==0
    ...
    plot(x/1000,400*ones(length(H)),'k--','linewidth',1.5)
        legend('total thickness','maximum extent',...
            'max thickness at 18 km')
    axis([0 (xmax/1000)+1 0 600])
