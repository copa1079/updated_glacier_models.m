...
%
%    1D FTCS STAGGERED GRID NUMERICAL MODEL: PINE CREEK PALEOGLACIER
%
%    SI units
%
...
%  set up distance array
    ...
    xmax  = 19466;     % make sure this matches your x-data !!!
       dx = xmax/step;
        x = dx/2:dx:xmax-(dx/2);
    xedge = 0:dx:xmax;
...
tributaries = 1.5; % how many main glacial valleys in the top?
W0    = 1000*tributaries;
Wstar = 10000;   % how quickly does the geometry shrink? FAST
Wmin  = 800;     % what is the minimum valley width?
...
Wedge = [Wedge(1) Wedge Wedge(end)];
    ...
    zb = transpose((PC_new_profile(1:N_data/step:end))); 
...
%  meteorology and mass balance
    ...
    sigma_ELA = 100;       % uncertainty in the ELA
 
    
    dbdz = 0.003;  % m/y/m, typically ~0.01, 
                   % for Arkansas case, we used 0.0027 (Brugger 2010)
                   
    bcap = 1.0;            % m/yr, usually 1.25-2.00
...
if rem(t(i),tplot)==0
    ...
    axis([-3 2 zmin-border zmax+border])
    ...
    plot(x/1000,(250+20+30)*ones(length(H)),'k--','linewidth',1.5) 
            % max thickness due to ice bulge (+30 m) 
            % and modern sedimentation (+20 m)
    legend('total thickness','maximum extent','theoretical max thickness')
    axis([0 xmax/1000 0 450])
...
%     axis([tmin-0.5 (tmax/1000)+tmin+0.5 0 max(x_terminus)])
    ...
    % PRESCRIBED CLIMATE
         ...
         plot((t/1000)+tmin,(min(ELA))*ones(size(t)),'r--','linewidth',1.5)
         plot((t/1000)+tmin,(max(ELA))*ones(size(t)),'r--','linewidth',1.5)
